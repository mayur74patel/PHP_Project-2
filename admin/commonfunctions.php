...
function addairlines($airl_name) {
    global $conn;
    $insert_query = "INSERT INTO airlines SET airl_name='" . $airl_name . "' ";
    $insert_res = mysqli_query($conn, $insert_query);
    if (mysqli_insert_id($conn)) {
        return TRUE;
    }
...
function addairport($vCategoryName,$city,$state,$country) {
    global $conn;
    $insert_query = "INSERT INTO airport SET air_name='" . $vCategoryName . "',city='" . $city . "',state='" . $state . "',country='" . $country . "' ";
    $insert_res = mysqli_query($conn, $insert_query);
    if (mysqli_insert_id($conn)) {
        return TRUE;
    }
...
function addflight($flight_name,$airl_name,$air_name,$source,$destination,$arrival,$depature,$duration,$status) {
    global $conn;
	
    $insert_query = "INSERT INTO flight SET flight_name='" . $flight_name . "',airl_name='" . $airl_name . "',airport_name='" . $air_name . "',destination='" . $destination . "',source='" . $source . "',arrival='" . $arrival . "',depature='" . $depature . "',duration='" . $duration . "',status='" . $status . "' ";
    $insert_res = mysqli_query($conn, $insert_query);
...
function addemployee($emp_name,$emp_phone_no,$emp_age,$emp_salary,$emp_address,$emp_job_type,$air_name) {
    global $conn;
	
    $insert_query = "INSERT INTO employee SET emp_name='" . $emp_name . "',emp_phone_no='" . $emp_phone_no . "',emp_age='" . $emp_age . "',emp_salary='" . $emp_salary . "',emp_address='" . $emp_address . "',emp_job_type='" . $emp_job_type . "',air_name='" . $air_name . "' ";
    $insert_res = mysqli_query($conn, $insert_query);
...
function getAllairlinesData() {
    ...
    if (mysqli_num_rows($select_res) > 0) {
        ...
        while ($row = mysqli_fetch_array($select_res)) {
            ...
            $temp[$i]['airl_name'] = $row['airl_name'];
            $i++;
        }
    }
    return $temp;
...
function getAllemployeeData() {
    ...
    if (mysqli_num_rows($select_res) > 0) {
        ...
        while ($row = mysqli_fetch_array($select_res)) {
            $temp[$i]['iCategoryID'] = $row['id'];
            $temp[$i]['air_name'] = $row['air_name'];
            $temp[$i]['emp_name'] = $row['emp_name'];
            $temp[$i]['emp_phone_no'] = $row['emp_phone_no'];
            $temp[$i]['emp_age'] = $row['emp_age'];
            $temp[$i]['emp_salary'] = $row['emp_salary'];
            $temp[$i]['emp_address'] = $row['emp_address'];
            $temp[$i]['emp_job_type'] = $row['emp_job_type'];
            $i++;
		}
    }
    return $temp;
...
function getAllflightData() {
    global $conn;
    $temp = array();
	$select_query = "SELECT *FROM flight ORDER BY id DESC";
    $select_res = mysqli_query($conn, $select_query);
    if (mysqli_num_rows($select_res) > 0) {
        $i = 0;
        while ($row = mysqli_fetch_array($select_res)) {
            $temp[$i]['iCategoryID'] = $row['id'];
            $temp[$i]['flight_name'] = $row['flight_name'];
            $temp[$i]['air_name'] = $row['airport_name'];
            $temp[$i]['airl_name'] = $row['airl_name'];
			$temp[$i]['source'] = $row['source'];
            $temp[$i]['destination'] = $row['destination'];
            $temp[$i]['arrival'] = $row['arrival'];
            $temp[$i]['depature'] = $row['depature'];
            $temp[$i]['duration'] = $row['duration'];
...
function getAllticketData() {
    global $conn;
    $temp = array();
	$select_query = "SELECT *FROM ticket ORDER BY id DESC";
    $select_res = mysqli_query($conn, $select_query);
    if (mysqli_num_rows($select_res) > 0) {
        $i = 0;
        while ($row = mysqli_fetch_array($select_res)) {
            $temp[$i]['iCategoryID'] = $row['id'];
            $temp[$i]['pass_name'] = $row['pass_name'];
            $temp[$i]['pass_address'] = $row['pass_address'];
			$temp[$i]['pass_age'] = $row['pass_age'];
            $temp[$i]['ticket_no'] = $row['ticket_no'];
            $temp[$i]['ticket_type'] = $row['ticket_type'];
            $temp[$i]['pass_mo_no'] = $row['pass_mo_no'];
            $temp[$i]['flight_name'] = $row['flight_name'];
            $temp[$i]['pass_email'] = $row['pass_email'];
			$temp[$i]['ticket_price'] = $row['ticket_price'];
...
function getAllairportData() {
    ...
    if (mysqli_num_rows($select_res) > 0) {
        ...
        while ($row = mysqli_fetch_array($select_res)) {
            $temp[$i]['iCategoryID'] = $row['id'];
            $temp[$i]['air_name'] = $row['air_name'];
            $temp[$i]['city'] = $row['city'];
            $temp[$i]['state'] = $row['state'];
            $temp[$i]['country'] = $row['country'];
            $i++;
        }
    }
    return $temp;
